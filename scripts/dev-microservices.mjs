...
const processes = [
  ["backend", "npm.cmd", ["--prefix", "backend", "run", "dev"], process.env],
  [
    "identity",
    "npm.cmd",
    ["--prefix", "services/identity-service", "run", "dev"],
    { ...process.env, IDENTITY_SERVICE_PORT: "4100", SERVICE_INTERNAL_TOKEN: internalServiceToken }
  ],
  [
    "listing",
    "npm.cmd",
...
].map(([name, command, args, environment]) => {
