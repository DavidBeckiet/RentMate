        ...
        text: `
          ...
          WHERE searches.is_active = true
            AND (
              ...
              OR strpos(lower($5::text), lower(searches.q)) > 0
            )
            AND (searches.area_name IS NULL OR strpos(lower($5::text), lower(searches.area_name)) > 0)
            ...
            AND (searches.property_type_code IS NULL OR searches.property_type_code = $6)
            AND searches.amenity_codes <@ $7::text[]
            AND (
              searches.mode = 'ordinary'
              OR (
                searches.mode = 'bounds'
                AND $8::double precision BETWEEN searches.south AND searches.north
                AND $9::double precision BETWEEN searches.west AND searches.east
              )
              OR (
                ...
                AND 2 * 6371.0088 * asin(
                  sqrt(
                    least(
                      1.0,
                      power(sin(radians($8::double precision - searches.center_lat) / 2), 2)
                      + cos(radians(searches.center_lat))
                      * cos(radians($8::double precision))
                      * power(sin(radians($9::double precision - searches.center_lng) / 2), 2)
                    )
                  )
                ) <= searches.radius_km
              )
