...
import type { ContactRepository, Inquiry, InquiryMessage } from "../src/modules/contact/repositories/contact-repository.js";
import type {
  ContactBlockState,
  ContactReport,
  ContactReportEvent,
...
function createHarness(): SafetyHarness {
  ...
  const executor: SqlExecutor = { query: async () => { throw new Error("SQL is not used by the safety harness."); } };
  const repository = {
    ...
    appendMessage: async (_executor: SqlExecutor, input: { readonly senderRole: "TENANT" | "LANDLORD"; readonly body: string }) =>
    ...
    createNotification: async () => ({ id: 1, eventType: "MESSAGE_CREATED" as const, inquiryId: inquiry.id, listingId: null, resourcePath: "/inquiries/9", isRead: false, createdAt }),
  ...
  const blockState = (currentUserId: number, otherUserId: number): ContactBlockState => {
    ...
    return Object.freeze({ blockedByCurrentUser: current, blockedByOtherUser: other, canSendMessage: !current && !other });
  };
  const reportValue = (reportId: number, reporterId: number, input: { readonly category: ContactReport["category"]; readonly details: string | null; readonly messageId: number | null }): ContactReport =>
    Object.freeze({
      id: reportId,
      ...
      reporterId,
      message: input.messageId === null ? null : Object.freeze({ id: input.messageId, senderRole: "LANDLORD", body: "Chuyển khoản trước.", createdAt }),
  ...
  const safetyRepository = {
    getBlockState: async (_executor: SqlExecutor, currentUserId: number, otherUserId: number) => blockState(currentUserId, otherUserId),
    isPairBlocked: async (_executor: SqlExecutor, first: number, second: number) => blocks.has(blockKey(first, second)) || blocks.has(blockKey(second, first)),
    createBlock: async (_executor: SqlExecutor, input: { readonly blockerId: number; readonly blockedId: number }) => { blocks.add(blockKey(input.blockerId, input.blockedId)); },
    deleteBlock: async (_executor: SqlExecutor, blockerId: number, blockedId: number) => { blocks.delete(blockKey(blockerId, blockedId)); },
    messageBelongsToInquiry: async (_executor: SqlExecutor, inquiryId: number, messageId: number) => inquiryId === inquiry.id && messageId === 3,
    createReport: async (_executor: SqlExecutor, inquiryId: number, reporterId: number, input: { readonly category: ContactReport["category"]; readonly details: string | null; readonly messageId: number | null }) => {
      if ([...reports.values()].some((report) => report.inquiryId === inquiryId && report.reporterId === reporterId && ["OPEN", "INVESTIGATING"].includes(report.status))) {
    ...
    listReports: async (_executor: SqlExecutor, input: { readonly status: ContactReport["status"] }) => [...reports.values()].filter((report) => report.status === input.status),
    findReport: async (_executor: SqlExecutor, reportId: number) => reports.get(reportId) ?? null,
    updateReportStatus: async (_executor: SqlExecutor, reportId: number, status: ContactReport["status"], adminId: number, note: string | null) => {
      const current = reports.get(reportId);
      if (!current) throw new Error("missing report");
      const updated = Object.freeze({ ...current, status, assignedAdminId: adminId, resolutionNote: status === "RESOLVED" || status === "DISMISSED" ? note : null, resolvedAt: status === "RESOLVED" || status === "DISMISSED" ? createdAt : null });
  ...
  const identityAccountClient: Pick<IdentityAccountClient, "loadProfilesByIds"> = {
    loadProfilesByIds: async (ids) => ids.map((id) => Object.freeze({ id, role: id === admin.userId ? "ADMIN" as const : id === landlord.userId ? "LANDLORD" as const : "TENANT" as const, email: `${id}@example.com`, phone: null, isActive: true }))
  };
...
test("enforces two-way blocks, report dedupe, participant ownership and admin transitions", async () => {
  ...
  await assert.rejects(() => harness.service.sendMessage(landlord, harness.inquiry.id, "hello"), (error: unknown) => error instanceof ApplicationError && error.code === "CONCURRENT_MODIFICATION");
  ...
  const report = await harness.service.createContactReport(tenant, harness.inquiry.id, { category: "FRAUD", details: "payment request", messageId: 3 });
  assert.equal(report.status, "OPEN");
  await assert.rejects(() => harness.service.createContactReport(tenant, harness.inquiry.id, { category: "SPAM", details: null, messageId: null }), (error: unknown) => error instanceof ApplicationError && error.code === "CONCURRENT_MODIFICATION");
  await assert.rejects(() => harness.service.createContactReport(outsider, harness.inquiry.id, { category: "SPAM", details: null, messageId: null }), (error: unknown) => error instanceof ApplicationError && error.code === "RESOURCE_NOT_FOUND");
  const adminPage = await harness.service.listContactReports(admin, { status: "OPEN", category: null, page: 1, pageSize: 20, offset: 0 });
  assert.equal(adminPage.data[0]?.reporter.email, "11@example.com");
  const investigating = await harness.service.updateContactReportStatus(admin, report.id, { status: "INVESTIGATING", note: null });
  assert.equal(investigating.status, "INVESTIGATING");
  const resolved = await harness.service.updateContactReportStatus(admin, report.id, { status: "RESOLVED", note: "Đã xử lý" });
