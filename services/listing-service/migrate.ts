...
const migrationRunner = createMigrationRunner("Listing");

function logPlan(plan: MigrationPlan): void {
  const logger = createLogger("info");
  logger.info("Listing migration plan prepared", {
    mode: plan.mode,
  ...
  for (const migration of plan.migrations) {
    logger.info("Listing migration selected", {
...
async function migrate(): Promise<void> {
  ...
  try {
    ...
    logger.info("Listing migration plan completed", {
...
void migrate().catch((error: unknown) => {
  ...
  if (error instanceof EnvironmentConfigurationError) {
    ...
    logger.error("Listing migration execution failed", {
    ...
    logger.error("Listing migration command validation failed", { reason: error.message });
  } else {
