...
function validateAbsentBody(value: unknown): void {
  if (value !== undefined) throwValidationIssue("body", "INVALID_VALUE", "body must be omitted.");
}

export function createConfirmOwnerListingAvailabilityHandler(service: ListingAvailabilityService): RequestHandler {
  return (request, response, next): void => {
    void (async () => {
      ...
      const listing = await service.confirmOwnedAvailability(principal, listingId);
