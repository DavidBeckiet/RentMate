...
import { validateListingBusinessStatusInput } from "../validations/business-status-validation.js";
...
export function createUpdateOwnerBusinessStatusHandler(service: ListingBusinessStatusService): RequestHandler {
  return (request, response, next): void => {
    void (async () => {
      const principal = requirePrincipal(request);
      const listingId = parseListingId(request.params.listingId);
      validateQueryKeys(request.query, []);
      const input = validateListingBusinessStatusInput(request.body);
      const listing = await service.updateOwnedBusinessStatus(principal, listingId, input);
