...
const requestFeedbackMessages: Record<RequestFeedbackKind, string> = {
  ...
  cancelled: "Yêu cầu đã được đóng.",
  ...
  return (
    <div className={styles.managed} data-editable={editable} data-editing={editing}>
      <Card className={`rm-roommate-card-static space-y-5 ${styles.overview}`}>
          ...
          <div className="space-y-3 border-t border-border pt-4">
            ...
            {selectedListing?.id === initialListingId ? (
              <div className="rm-roommate-callout space-y-3 text-ui-sm leading-6" data-tone="accent">
                <p>
                  Bạn vừa chọn phòng từ trang chi tiết. Có thể gắn thẳng vào yêu cầu này hoặc chọn một phòng khác trước khi tiếp tục.
                </p>
                <div className="flex flex-wrap gap-2">
                  <Button pending={pendingAction === "link"} pendingLabel="Đang liên kết…" onClick={() => void link()}>
                    <Icon name="arrowUpRight" className="h-4 w-4" /> Gắn phòng này vào yêu cầu
                  </Button>
        ...
        {initialListingId && request.status !== "OPEN" && request.listingId !== initialListingId ? (
          <p className="rm-roommate-callout text-ui-sm leading-6" data-tone="info" role="status">
            Phòng từ trang bạn vừa xem chưa được liên kết. Yêu cầu này không còn mở nên không thể thay đổi phòng tại đây; trạng thái và kết nối hiện tại vẫn được giữ nguyên.
          </p>
        ) : null}
        {request.note ? (
        ...
        <RoommateRequestListingSummary request={request} onChangeListing={editable ? () => setEditing(true) : undefined} />
        ...
        {editable ? (
          <div className="flex flex-wrap gap-2 border-t border-border pt-4">
            ...
            >
              {editing ? "Thu gọn chỉnh sửa" : "Chỉnh sửa yêu cầu"}
            </Button>
          ...
          <section className="space-y-3 border-t border-border pt-4" aria-label="Chọn phòng cân nhắc">
            <div>
              ...
              <p className="mt-1 text-ui-sm leading-6 text-muted-foreground">
                Không bắt buộc. Chỉ thêm ngữ cảnh để mọi người cùng trao đổi; liên kết không giữ chỗ và không xác nhận thuê.
              </p>
            </div>
            <RoommateListingPicker
              selected={selectedListing}
            ...
            <div className="grid gap-2 sm:flex sm:flex-wrap">
              {selectedListing && selectedListing.id !== request.listingId ? (
                ...
                <span className="inline-flex min-h-11 items-center gap-2 text-ui-sm font-semibold text-success-foreground" role="status">
      ...
      <Dialog
        ...
        description="Phòng mới sẽ thay phòng đang gắn với yêu cầu ở ghép của bạn."
      ...
      >
        <div className="space-y-4">
          ...
          <p className="rm-roommate-callout text-ui-sm leading-6" data-tone="info">
            Chỉ tin phòng được gắn thay đổi; ngân sách, khu vực và ghi chú trong yêu cầu của bạn vẫn giữ nguyên.
          </p>
          {error ? (
            <p role="alert" className="rm-roommate-callout text-ui-sm font-semibold text-danger" data-tone="danger">
              {error}
      ...
      <Dialog
        open={confirmCancel}
        title="Đóng yêu cầu ở ghép?"
        description="Việc đóng yêu cầu sẽ kết thúc các lời quan tâm đang chờ và không thể mở lại yêu cầu này."
        onClose={() => setConfirmCancel(false)}
      >
        <div className="space-y-4">
          <div className="rm-roommate-callout" data-tone="danger">
            <p className="text-ui-sm font-semibold leading-6 text-foreground">
              Nếu muốn tìm tiếp, bạn cần tạo yêu cầu mới. Các lời quan tâm cũ sẽ không được khôi phục.
            </p>
          </div>
          {error ? (
            <p role="alert" className="rm-roommate-callout text-ui-sm font-semibold text-danger" data-tone="danger">
