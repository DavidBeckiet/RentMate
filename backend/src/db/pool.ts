...
export type PostgresPoolFactory = (config: BackendConfig["database"], logger: Logger) => Pool;
...
export function buildPostgresPoolConfig(config: BackendConfig["database"]): PoolConfig {
  return {
    host: config.host,
    port: config.port,
...
export function createPostgresPool(config: BackendConfig["database"], logger: Logger): Pool {
...
export interface RuntimePoolManager {
  getPool(config: BackendConfig["database"], logger: Logger): Pool;
  close(): Promise<void>;
}
...
export function getRuntimePool(config: BackendConfig["database"], logger: Logger): Pool {
